...
class RestActionDetector:
    ...
    def _drink_water_detection(self, coords):
        ...
        # 检查所需关键点是否存在
        required_points = [self.mp_pose.PoseLandmark.LEFT_WRIST, 
        ...
        if all(p.value in coords for p in required_points):
            ...
            ml = coords[self.mp_pose.PoseLandmark.LEFT_EYE.value]
            mr = coords[self.mp_pose.PoseLandmark.RIGHT_EYE.value]
            
            # 计算手腕到嘴的距离
            dist_l = np.sqrt((lw[0]-ml[0])**2 + (lw[1]-ml[1])**2)
            dist_r = np.sqrt((rw[0]-mr[0])**2 + (rw[1]-mr[1])**2)
    ...
    def _leave_seat_detection(self, coords):
        ...
        if coords:  # 检测到人体
            ...
            return False, f"left for {int(absence_duration)}/5秒"
